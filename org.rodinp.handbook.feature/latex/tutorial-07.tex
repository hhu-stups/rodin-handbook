...
\marginpar{Please check English grammar!}
We start with the traffic lights for the pedestrians. It consist of just two colors (red and green), exactly one of them is shown.  We introduce a new variable to represent which light is shown called \textsf{peds\_color}, with corresponding invariant and an initialization (the changes are shown in the following):
