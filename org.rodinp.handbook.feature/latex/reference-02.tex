...
%TODO Maybe this can be removed later, but the screenshots from the previous
% section should not appear in this section
\clearpage
...
\index{proof obligation!axiom as theorem}
...
\index{proof obligation!well-definedness of an axiom}
...
\index{proof obligation!invariant as theorem}
...
\index{proof obligation!well-definedness of an invariant}
...
If the event's guard is enabled, every action must be feasible.
  ...
  \index{proof obligation!action feasibility}
...
\index{proof obligation!guard strengthening}
...
\index{proof obligation!action simulation}
...
\index{status of an event!ordinary}\index{status of an event!anticipated}\index{status of an event!convergent}
(This section will be written in a later iteration.)
...
\end{quote}
\marginpar{if this paragraph from the wiki is used, the section labels 1 and 2 should be replaced by references to corresponding sections in th handbook}
...
\marginpar{this section has been migrated from the wiki. Might need some revision of content}
\marginpar{maybe Well-definedness should be formally defined in the first paragraph?}
...
In other places (but not in this handbook), 1 + 1 = 2 is called a predicate and 1 + 1 an expression.

\marginpar{Should D-Well-Definedness and Definition be their own subsubsections?}
...
\marginpar{have the Inference Rules and All Rewrite Rules been migrated to the handbook? If so, add ref. If not, add link to wiki?}
...
\marginpar{Should Distinction between D and L
