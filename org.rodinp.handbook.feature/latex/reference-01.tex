...
When opening a machine or context file, some wizards for creating Event-B model elements are available for the user. The different wizards are described in section \ref{ref_01_the_eventb_editor}.

\subsubsection{Tool bar}

The tool bar provides short cuts for familiar commands like save, print, undo and redo. The tool bar also provides short cuts to the wizards for creating elements like axioms, constants, enumerated sets, etc., which are described in section \ref{ref_01_the_eventb_editor}.
...
You can then enter the names of a constant and an axiom which can be used to define the constant's type. By pressing \textsf{More Axm.} button you can enter additional axioms. To add more constants, press the \textsf{Add} button. When you have finished, press the \textsf{OK} button.
...
To activate the \textsf{New Events Wizard}, press the \icon{rodin/newevt_edit.png} button located in the tool bar. Pressing the button brings up the window shown in figure \ref{fig_ref_01_eventb_editor16}.
...
The \textsf{Synthesis} tab for machines is very similar to the one of contexts except that you have a global view of your machine's elements (refined machine/seen context/variable/invariant/event/variant).
...
\marginpar{Are the squares next to the nodes from the up-to-date version of Rodin? I have triangles}
...
\begin{itemize}
    ...
    \item (p0) external provers can be invoked from the drop-down list to test sequents using different forces.
...
ML (mono-lemma) prover appears in the drop-down list under the button (pp) as M0, M1, M2, M3, and ML. The different configuration (e.g., M0) refer to the proof force of the ML prover. All hypotheses are passed to ML.
...
\begin{itemize}
	...
	\item P1 performs a lasoo operation on the selected hypotheses and the goal and uses the resulting hypotheses.
...
The list on the right represents the available and unselected tactics. The list of the left displays the profile contents and shows the selected tactics that will be applied from the top to the bottom. The user can choose a tactic from the list on the left and hit the "$>>$" button to select it or unselect some tactics from the list of the right using the "$<<$" button. The user can re-order the priority of the selected tactic using the "Up" and "Down" button. By clicking on "Finish", the profile will be saved and available for use in the auto and post tactics.
