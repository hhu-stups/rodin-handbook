...
\marginpar{please review}
In this tutorial, we will create a model of the well known Agatha puzzle. 
We use this instead of the already introduced traffic light example because it provides
 more possibilities to apply Event-B's logic and to use operations on relations.
...
We mark the axiom \texttt{solution} as a theorem (\ref{tut_concepts_contexts}) by clicking on the \textsf{not theorem} button\footnote{Yes, this sounds wrong.  However, the button is a toggle button, so after clicking it, the label changes to ``theorem''.  The button always displays the current state.} as shown in figure \ref{fig_tut_05_mark_theorem}.
...
But at this point of the tutorial we do not want to go into the details of proof yet.
