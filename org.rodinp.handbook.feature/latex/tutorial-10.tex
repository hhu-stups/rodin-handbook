...
\item The first is the well-definedness condition of $sit(x) = outside$. $sit$ must be a function and $x$ is in its domain.
  This is easy to prove since $sit$ is a total function (\ref{relations}). Press the \icon{rodin/p1.png} button to verify it. \marginpar{is the icon for p1 too small? Should we use \textsf{p1} instead?}
