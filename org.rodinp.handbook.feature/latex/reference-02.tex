...
\paragraph{If the variant is numeric,}
...
\begin{tabular}{lll}
  ...
  finiteness for a set variant  & \eventbpo{FIN} & \ref{po_set_variant} \\
  natural number for a numeric variant & \eventbpo{event/NAT} & \ref{po_numeric_variant} \\
  decreasing of variant & \eventbpo{event/VAR} &\ref{po_numeric_variant} \\
  \hline
\end{tabular}
