...
Every action $a$ with label $\eventbpo{actionlabel}$ of an event must be well-definend.
...
If the event's guard is enabled, every action must be feasable.
  ...
  set $E$ is not empty:
  \pode{Action feasability (for $\bcmin$)}{\eventbpo{eventlabel/actionlabel/FIS}}%
  {$A(\allconstants) \land I(\allconstants,\allvariables) \land G(\allconstants,\concvariables,\concparameters) \limp %
  ...
  \pode{Action feasability (for $\bcmsuch$)}{\eventbpo{eventlabel/actionlabel/FIS}}%
  {$A(\allconstants) \land I(\allconstants,\allvariables) \land G(\allconstants,\concvariables,\concparameters) \limp %
...
For the initialisations we have the following special cases:
  \pode{Action feasability (for $\bcmin$ in initialisation)}{\eventbpo{INITIALISATION/actionlabel/FIS}}%
  {$A(\allconstants) \limp  E(\allconstants) \neq \emptyset$}
  \pode{Action feasability (for $\bcmsuch$ in initialisation)}{\eventbpo{INITIALISATION/actionlabel/FIS}}%
...
\pode{Witness feasability}{\eventbpo{eventlabel/witnesslabel/WFIS}}%
