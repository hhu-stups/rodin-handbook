...
A useful tool for understanding and debugging a model is a model checker like ProB (\ref{prob}).  You can install ProB from the ProB Update Site, directly from Rodin.  Just select \textsf{Help $\rangle$ Install New Software... $\rangle$ Event-B Component} and select ``ProB'' from the dropdown. You should see ``ProB for Rodin2`` as an installation option, which you can then install using the normal Eclipse mechanism.
