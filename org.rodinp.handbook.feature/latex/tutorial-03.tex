...
This editor is \textit{form-based}.  This means that it can be modified by using controls (i.e. text fields, dropdowns, etc.) to input information. 
...
Event-B provides the build-in datatype \texttt{BOOL} amongst others (Section \ref{data_types}).
...
To prevent the invariant from being violated (and therefore to allow all proof obligations to be discharged), we need to strengthen the guards (Section \ref{guards}) of the events.

\warning{Before looking at the solution, try to fix the model yourself.}

\marginpar{Does ProB only work for Rodin 2?}
