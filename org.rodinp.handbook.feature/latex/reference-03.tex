...
\item \index{pair!as type} $\alpha\cprod\beta$ contains the pairs where the first element is of type $\alpha$ and the
...
\begin{rodinrefentry}
  \rrdesc
    ...
    The existantial quantification $\forall x_1\ldots,x_n~\qdot~\predp$ is true if there is a valuation of
...
\begin{rodinrefentry}
  \rrdesc
    $\exprs\subseteq \exprt$ checks if $\exprs$ is a subset of $\exprt$, i.e. if all elements of $\exprs$ occurr in $\exprt$.
...
\begin{rodinrefentry}
  \rrdesc
    ...
    The intersection $\exprs\binter \exprt$ denotes the set that contains all elment that are in both $\exprs$ and $\exprt$.
...
\begin{rodinrefentry}
  \rrdesc
    ...
    Analougously is $\Union x_1\ldots,x_n~\qdot~\predp~|~\expre$ the intersection of all values of $\expre$ for
...
\begin{rodinrefentry}
  \rrdesc
    ...
    second of $\exprt$. $\exprs\rel \exprt$ is just an abreviation for $\pow(\exprs\rel \exprt)$.
...
\begin{rodinrefentry}
  \rrdesc
    ...
    Likewise the range $\ran(\exprlr)$ is the set of elements of $\exprt$ where at least one element
    of $\exprs$ relates to by $\exprlr$.
  \rrdef
