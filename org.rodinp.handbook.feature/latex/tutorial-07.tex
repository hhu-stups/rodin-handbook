...
We start with the traffic lights for the pedestrians which consist of just two colors (red and green).  We introduce a new variable to represent the traffic light color called \textsf{peds\_color}, with corresponding invariant and an initialization (the changes are shown in the following):
...
\INVARIANTS
	\begin{description}
		\nItemX{ inv4 }{ peds\_color \in  \{  red, green \}  }
	\end{description}
\EVENTS
	\INITIALISATION
		\\\textit{extended}		
...
Next, we will create a gluing invariant (\ref{gluing_invariant}) that associates \textsf{peds\_go} from the abstract machine with the variable \textsf{peds\_color} that we just created.  The gluing invariant will map \textsf{TRUE} to \textsf{green} and \textsf{FALSE} to \textsf{red}:

\pencil{
\begin{description}
\INVARIANTS
	\begin{description}
		\nItemX{ gluing }{ peds\_go = TRUE \leqv  peds\_color = green }
	\end{description}
\end{description}
}
...
\EVENTS
	...
	\REF {set\_cars}
		...
		\WhereGrd
			...
			\nItemX{ grd2 }{ new\_value = TRUE \limp  peds\_color = red }
			\end{description}
		\ThenAct
			\begin{description}
...
\begin{description}
	...
	\REF {set\_cars}
		...
		\WhereGrd
			...
			\nItemX{ grd2 }{ green \in  new\_value\_colors \limp  peds\_color = red }
			\end{description}
		\Witnesses
			\begin{description}
...
\INVARIANTS
	\begin{description}
		\nItemX{ inv4 }{ peds\_color \in  \{  red, green \}  }
		\nItemX{ inv5 }{ cars\_colors \subseteq  COLORS }
		\nItemX{ gluing\_peds }{ peds\_go = TRUE \leqv  peds\_color = green }
		\nItemX{ gluing\_cars }{ cars\_go = TRUE \leqv  green \in  cars\_colors }
	\end{description}
\EVENTS
	\INITIALISATION
