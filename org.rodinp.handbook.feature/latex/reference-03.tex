...
\begin{rodinrefentry}
  \rrdesc
    $(\lambda~x~\qdot~P~|~E~)$ is a function that maps an ``input'' $x$ to
    a result $E$ such that $P$ holds. 

    TODO (esp. pattern instead of identifiers)
  \rrdef
    $(\lambda~x_1,\ldots,x_n~\qdot~P~|~E~) = \{~(x_1\mapsto\ldots\mapsto x_n)\mapsto E~|~P~\}$
  \rrtypes
    $(\lambda~x_1,\ldots,x_n~\qdot~P~|~E~)\in\pow(~(\alpha_1\cprod\ldots\cprod\alpha_n)\cprod\beta~)$ with the identifiers $x_1\in\alpha_1$, \dots, $x_n\in\alpha_n$, $P$ being a predicate
    and $E\in\beta$.
  \rrwd
    $\wdl(\lambda~x_1,\ldots,x_n~\qdot~P~|~E~)\defi
    \forall x_1,\ldots,x_n~\qdot~\wdl(P)\land (P\limp \wdl(E))$
\end{rodinrefentry}
