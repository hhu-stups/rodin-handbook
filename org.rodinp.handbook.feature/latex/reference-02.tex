...
If the event's guard is enabled, every action must be feasible.
  ...
  The feasibility operator $\actfis$ is defined in Section~\ref{well_definedness}. 
...
This is a brief overview about the different proof obligations that are generated.
The user can use this table to identify a specific proof obligation. For further information, a link to the relevant reference section is provided.
...
\item[Concrete Variables] Concrete variables that are defined in the machine itself can be used. This does \emph{not}
...
\item[Concrete Parameters] Parameters that are defined in the event itself can be used. This does \emph{not} include parameters of refined events.
