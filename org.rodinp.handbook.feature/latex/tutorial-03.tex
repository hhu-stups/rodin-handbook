...
\begin{itemize}
	\item We will model the signals with boolean values to indicate ``stop'' (false) and ``go'' (true).  We do not model colors (yet).
	\item We will not model the push button yet.
\end{itemize}
...
\label{camille}
\marginpar{Should the link for Camille be in a footnote?}
...
Back to the problem: Our objective is to build a simplified traffic light controller as described in \ref{tutorial_03}.  We start with the model state.  Two traffic lights will be modelled and we will therefore create two variables called  \texttt{cars\_go} and \texttt{peds\_go}.  
...
Event-B provides the build-in datatype \texttt{BOOL} amongst others (\ref{datatypes}).
...
To fix this problem, expand the \textsf{EVENTS} section and then the INITIALIZATION event (\ref{initialization}).  Add two elements in the \textsf{THEN} block.  These are actions that also have labels.  In the action fields, enter $cars\_go :=  FALSE$ and $peds\_go :=  FALSE$.
...
Our traffic light controller cannot yet change its state.  To make this possible, we create events (\ref{event}). We will first consider the traffic light for the pedestrians, and we will create two events. One will set it to ``go'' and one will set it to ``stop''.
...
Obviously, this invariant can be violated, and Rodin informs us of this.  The \textsf{Event-B Explorer} (\ref{eventb_explorer}) provides this information in various ways.  Go to the explorer and expand the project (\texttt{tutorial-03}), the machine (\texttt{mac}) and the entry ``Proof Obligations''.  You should see four proof obligations, two of which are discharged and two of which are not discharged.

\info{Make sure that you understand the proof obligation labels (\ref{po_labels}).  The proof obligations can also be found via other entries.  Elements that have non-discharged proof obligations as children are marked with a small question mark.  For instance, \texttt{inv3} has all proof obligations as children, while the event \texttt{set\_cars} has one.}
