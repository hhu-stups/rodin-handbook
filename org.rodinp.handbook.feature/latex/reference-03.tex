...
When we introduce new identifiers while expressing an operator (e.g. a set comprehension), we assume that the new identifier does not occur free in the rewritten expressions \marginpar{Does this paragraph make sense?} (i.e. any new identifiers that are used to express an operator must either be bound to the original operator or must not appear in the original operator at all). 

\info{For more information, download the four-page \file{EventB-Summary.pdf}{Event-B Cheat Sheet}, which provides a concise summary of the Event-B mathematical toolkit.  We would like to thank Ken Robinson for making it available.}
