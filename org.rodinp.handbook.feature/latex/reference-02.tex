...
\info{Please note that Rodin often does not show a proof obligation if it
  is obviously true.
  If you expect a proof obligation that Rodin does not show, you can enforce
  that the proof obligation is showed by changing the model temporarily 
  in a way that the proof cannot be proven.
  E.g. you can introduce a division by zero to see the corresponding
  well-definedness condition.}

We first describe context and machines and how their consistency is proven.
...
We write $\absinvariant$ to denote the conjunction of all invariants
of the abstract machines and $\concinvariant$ for the conjunction of the machine's invariant.
...
\pode{Witness feasibility for a parameter $p$}{\eventbpo{eventlabel/$p$/WFIS}}%
  {$\axioms \land \absinvariants \land \concinvariants \land \bapred \limp
...
See \ref{witnesses} for details on witnesses resp. \ref{assignments} for the scope
