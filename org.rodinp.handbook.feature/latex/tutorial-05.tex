...
\marginpar{Update screenshot for Rodin 2.4}
...
All axioms are set to ``not theorem'' when they are created. But we need the solution to be a theorem so that we can proove it. In order to do this, place your cursor in the box where \textsf{not theorem} is shown to the left of the axiom \textsf{solution}. This box will automatically change to \textsf{theorem}, and you will have set the axiom as a theorem as shown in Figure \ref{fig_tut_05_mark_theorem}.

\marginpar{Screenshot must be adapted to Rodin v.2.4 or taken away altogether}
