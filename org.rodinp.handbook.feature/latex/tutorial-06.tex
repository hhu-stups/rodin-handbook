...
\marginpar{Does this sentence mean: The goal of such a proof is to assert that when all affected variables are replaced by new values from the actions, the invariant still is valid.}
The goal of such a proof is the affected invariant where all variables are replaced by
the new values from the actions. We can use the following hypotheses:
\begin{itemize}
\marginpar{Do the hypotheses mean: 1. The invariants are unchanged because we know that before the even the invariants were true, 2. The guards are valid because the even only occurs when the guards are valid. PLEASE CHECK! These paragraphs don't make sense.}
