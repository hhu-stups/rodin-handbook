...
When the an error is discovered in a project, a \icon{rodin/error_ovr.png}\marginpar{update icon for Rodin 2.4} marker appears next to the line with the problem in the editor view, and the faulty component in the \textsf{Event-B Explorer} is also similarly marked. The error itself (e.g. a syntax error) is shown in the Rodin Problems view.
...
You can then enter the names of the variable, what its state at initialization should be, and an invariant which defines its type. By pressing button \textsf{More Inv.}, you can enter additional invariants. To add more variables, press the \textsf{Add} button. When you're finished, press the \textsf{OK} button. 
...
\begin{itemize}
	...
	\item The \icon{rodin/p1.png} prover performs a lasoo operation on the selected hypotheses and the goal and uses the resulting hypotheses.
...
\marginpar{This window is actually described in much further detail in the section on preferences. The preferences shown in the current screenshot are no longer available.}
...
\begin{itemize}
	...
	\item Automatically if such preference is activated. (Auto-tactic will then run after each proof step and post-tactic will run after each step and rebuild). Post-tactics and auto-tactics only need to be activated in order to run automatically (see previous sections on how to activate auto- and post-tactics). 
\end{itemize}
...
The user can select profiles locally for a project. To do so, select the \textsf{Prefix Settings} that is available in the menu that pops up after right-clicking on a project, and select the \textsf{Properties} item. You can also click on the \textsf{Configure project specific settings} link on the \textsf{Prefix Settings} preference page. In this case, one will have to choose the project for which the prefixes should be set up. This is allowed via a specific project selection dialog. After the project selection, a dialog for prefix settings opens for the selected project. \marginpar{Is this current for Rodin 2.4?}
...
\marginpar{update for Rodin 2.4}
