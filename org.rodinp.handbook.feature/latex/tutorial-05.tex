...
Next, create a new Event-B Component. The process for creating a context is similar to the process for creating a machine (Section \ref{tut_first_machine}), but this time use \texttt{agatha} as the component name and select the \textsf{Context} (\ref{context}) option in order to create a Context file instead of a Machine (\ref{machine}) file.
