...
\begin{description}
\item[Refines] \index{refines} Optionally a machine can refine another one.
...
Here we give just a brief overview over the concept of refinement. Later in Section \ref{tut_expanding_traffic_light_system}
...
To ensure that the concrete machine does only what is allowed by the abstract one, we must show two things:
\begin{itemize}
