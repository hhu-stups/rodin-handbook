...
\info{It exists an wizard (\ref{wizard}) which creates the contants, the set and the axiom automatically. Therefore, click on the \textsf{New Enumarated Set Wizard} tool bar item. This will bring up the wizard (\ref{wizard}) where we can enter the name of the set and the constants in the corresponding text fields. The wizard will create the enumarted set, the constants and the axiom automatically.}
...
We mark the axiom (\ref{axiom}) \texttt{solution} as a theorem (\ref{theorem}) by clicking on the \textsf{not theorem} button:

\begin{center}
	\includegraphics{img/tutorial/tut_05_agatha3.png}
\end{center}

Theorems (\ref{theorem}) describe properties that are expected to be able to be derived from the axioms (\ref{axiom}).

TODO: explain Theorems and mention Well-Definedness! 

This concludes the tutorial. The following section \ref{tut_05_final_context} shows the complete Context (\ref{context}).
