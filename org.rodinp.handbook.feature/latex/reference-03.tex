...
\begin{rodinrefentry}
  \rrdesc
    ...
    type is specified by the type expression (\ref{data_type}) $\alpha$.

    $E$ is restricted to \ldots (TODO).

    This expression is only needed when the type of an expression is otherwise not inferable.
    $\boftype$ is rarely used, mostly inside proofs, not models (TODO: Is that so?).
  \rrdef
    $E\boftype\alpha = E$
  \rrtypes
